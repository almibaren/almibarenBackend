...
if (count($pPopId) > 0) {
    foreach ($pPopId as $item) {
        $sqlPop = "SELECT DISTINCT p.id, p.nombre,i.url,hp.descuento,pr.precio
  FROM  producto p 
...
AND g.nombre=? OR g.nombre=? OR g.nombre=?";
        ...
        foreach ($pRecId as $item) {
            $sqlRec = "SELECT DISTINCT p.id, p.nombre,i.url,hp.descuento,pr.precio
  FROM  producto p 
  INNER JOIN historicoPrecio hp ON p.id = hp.idProducto
  INNER JOIN precio pr ON hp.idPrecioProducto = pr.id
  INNER JOIN imagen i ON p.id = i.id
...
if (count($pRateId) > 0) {
    foreach ($pRateId as $item) {
        $sqlRate = "SELECT DISTINCT p.id, p.nombre,i.url,hp.descuento,pr.precio
  FROM  producto p 
  INNER JOIN historicoPrecio hp ON p.id = hp.idProducto
  INNER JOIN precio pr ON hp.idPrecioProducto = pr.id
  INNER JOIN imagen i ON p.id = i.id
...
if (count($pDealId) > 0) {
    foreach ($pDealId as $item) {
        $sqlDeal = "SELECT DISTINCT p.id, p.nombre,i.url,hp.descuento,pr.precio
  FROM  producto p 
  INNER JOIN historicoPrecio hp ON p.id = hp.idProducto
  INNER JOIN precio pr ON hp.idPrecioProducto = pr.id
  INNER JOIN imagen i ON p.id = i.id
